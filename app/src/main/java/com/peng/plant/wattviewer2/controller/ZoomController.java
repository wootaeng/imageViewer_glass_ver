...
//            p.setAlpha(100);  // 투명도 조정
            ...
            final float fix_dx = (float) (w * 1030.0 / getWidth()); //검은테두리 가로 (
            final float fix_dy = (float) (h * 500.0 / getHeight()); //검은 테두리 세로

            canvas.drawRect(fix_dx - 0.5f * w, fix_dy - 0.5f * h, fix_dx + 0.5f * w, fix_dy + 0.5f * h, p);
            Log.d(TAG, "dispatchDraw: fix_dx =" +fix_dx);
            Log.d(TAG, "dispatchDraw: fix_dy =" +fix_dy);

            p.setColor(Color.BLUE);

            canvas.drawRect(dx - 0.5f * w / zoom, dy - 0.5f * h / zoom, dx + 0.5f * w / zoom, dy + 0.5f * h / zoom, p);
            Log.d(TAG, "dispatchDraw: dx =" +dx);
            Log.d(TAG, "dispatchDraw: dy =" +dy);
//            canvas.setBitmap(ch);
